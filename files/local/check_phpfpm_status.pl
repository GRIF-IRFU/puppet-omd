...
my $o_host =        '127.0.0.1';  # hostname 
my $o_help=         undef;  # want some help ?
my $o_port=         undef;  # port
my $o_url =         '/php_status';  # url to use, if not the default
...
sub nagios_exit {
    ...
    if(!defined($silent)) {
        my $output = undef;
        $output .= sprintf('%1$s %2$s %3$s', $status, $nickname);
        if ($perfdata) {
            $output .= sprintf('%1$s', $perfdata);
        }
	$output .= sprintf($message);
        $output .= chr(10);
        print $output;
    }
...
$SIG{'ALRM'} = sub {
  nagios_exit($phpfpm,"2","ERROR: Alarm signal (Nagios timeout)");
};
...
sub check_options {
    ...
    if (defined ($o_help)) { 
        help();
        nagios_exit($phpfpm,"3","leaving","",1);
    }
    ...
    if ((defined($o_warn_p_level) && defined($o_crit_p_level)) &&
        ...
        nagios_exit($phpfpm,"3","Check warning and critical values for IdleProcesses (1st part of thresold), warning level must be > crit level!");
    }
    if ((defined($o_warn_m_level) && defined($o_crit_m_level)) &&
         (($o_warn_m_level != -1) && ($o_crit_m_level != -1) && ($o_warn_m_level >= $o_crit_m_level)) ) { 
        nagios_exit($phpfpm,"3","Check warning and critical values for MaxProcesses (2nd part of thresold), warning level must be < crit level!");
    }
    if ((defined($o_warn_q_level) && defined($o_crit_q_level)) &&
         (($o_warn_q_level != -1) && ($o_crit_q_level != -1) && ($o_warn_q_level >= $o_crit_q_level)) ) { 
        nagios_exit($phpfpm,"3","Check warning and critical values for MaxQueue (3rd part of thresold), warning level must be < crit level!");
    }
    # Check compulsory attributes
    if (!defined($o_host)) { 
        print_usage();
        nagios_exit($phpfpm,"3","-H host argument required");
    }
}
...
if ($response->is_success) {
    ...
    if ($response->header('Content-Type') =~ m/text\/html/) {
        nagios_exit($phpfpm,"2", "We have a response page for our request, but it's an HTML page, quite certainly not the status report of php-fpm");
    }
    ...
    {
        open ($FH, '<',$TempFile) or nagios_exit($phpfpm,"3","unable to read temporary data from :".$TempFile);
        $LastUptime = <$FH>;
        $LastAcceptedConn = <$FH>;
    ...
    open ($FH, '>'.$TempFile) or nagios_exit($phpfpm,"3","unable to write temporary data in :".$TempFile);
    print $FH "$Uptime\n"; 
    ...
    if (defined($o_crit_q_level) && (-1!=$o_crit_q_level) && ($MaxListenQueueNew >= $o_crit_q_level)) {
        nagios_exit($phpfpm,"2", "Max queue reached is critically high " . $InfoData,$PerfData);
    }
    if (defined($o_crit_m_level) && (-1!=$o_crit_m_level) && ($MaxChildrenReachedNew >= $o_crit_m_level)) {
        nagios_exit($phpfpm,"2", "Max processes reached is critically high " . $InfoData,$PerfData);
    }
    if (defined($o_crit_p_level) && (-1!=$o_crit_p_level) && ($IdleProcesses <= $o_crit_p_level)) {
        nagios_exit($phpfpm,"2", "Idle workers are critically low " . $InfoData,$PerfData);
    }
    # Then WARNING exits by priority
    if (defined($o_warn_q_level) && (-1!=$o_warn_q_level) && ($MaxListenQueueNew >= $o_warn_q_level)) {
        nagios_exit($phpfpm,"1", "Max queue reached is high " . $InfoData,$PerfData);
    }
    if (defined($o_warn_m_level) && (-1!=$o_warn_m_level) && ($MaxChildrenReachedNew >= $o_warn_m_level)) {
        nagios_exit($phpfpm,"1", "Max processes reached is high " . $InfoData,$PerfData);
    }
    if (defined($o_warn_p_level) && (-1!=$o_warn_p_level) && ($IdleProcesses <= $o_warn_p_level)) {
        nagios_exit($phpfpm,"1", "Idle workers are low " . $InfoData,$PerfData);
    }
    
    nagios_exit($phpfpm,"0",$InfoData,$PerfData);
    
} else {
    nagios_exit($phpfpm,"2", $response->status_line);
}
